/**
 * Is the walk ROUTE drawn, and does the number come from it?
 *
...
const DIST = path.join(ROOT, 'dist'); const PORT = 4497;
...
/* A route that bends round a block: 65 m east, 300 m north, 65 m back west.
   430 m of walking for a 300 m crow flight — that difference IS the report. */
const DLON = 65 / (111320 * Math.cos(HERE.lat * Math.PI / 180));
const WALK_PTS = [
  [HERE.lat, HERE.lon],
  [HERE.lat, HERE.lon + DLON],
  [HERE.lat + METRO.off / 111320, HERE.lon + DLON],
  [HERE.lat + METRO.off / 111320, HERE.lon],
];
// Valhalla is precision 6; ui/polyline.js only decodes, so the probe encodes.
function encode(pts, precision) {
  ...
  pts.forEach(([lat, lon]) => {
    ...
    chunk(la - pl); chunk(lo - pn); pl = la; pn = lo;
  });
  return out;
}
const SHAPE = encode(WALK_PTS, 6);
...
async function run(scheme) {
  ...
  // Valhalla, with a route that bends round a block: 430 m of walking for a
  // 300 m crow flight. That difference IS the report.
  let valhallaBody = null;
  await page.route('**/valhalla1.openstreetmap.de/**', route => {
    valhallaBody = JSON.parse(route.request().postData() || '{}');
    return route.fulfill({ status: 200, contentType: 'application/json',
      body: JSON.stringify({ trip: { legs: [{ shape: SHAPE }] } }) });
  });
  await page.route(/tiles|open-meteo|overpass|geoapify|mobility|realtime/, r => r.abort());
  ...
  const read = await page.evaluate(() => {
    ...
    return {
      ...
      gang: document.querySelectorAll('#board-map path').length,
      lagret: localStorage.getItem('default::t.walkDist'),
      dashed: Array.from(document.querySelectorAll('#board-map path'))
        .map(e => (e.getAttribute('stroke-dasharray') || '-') + ' ' + (e.getAttribute('d') || '').slice(0, 40)),
    };
  });
  console.log('\n══ ' + scheme + ' ══');
  ...
  console.log('  streker p\u00e5 kartet:', read.gang);
  (read.dashed||[]).forEach(d => console.log('     ', d));
  console.log('  lagret lengde   :', read.lagret);
  console.log('  costing         :', valhallaBody && JSON.stringify(valhallaBody.costing_options));
  read.reach.forEach(r => console.log('    igjen:', r));
  fs.mkdirSync('scratchpad/shots', { recursive: true });
  await page.screenshot({ path: 'scratchpad/shots/approach-' + scheme + '.png', animations: 'disabled' });
  await ctx.close();
}
