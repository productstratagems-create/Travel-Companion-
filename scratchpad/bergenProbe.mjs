...
const DIST = path.join(ROOT, 'dist'); const PORT = 4501;
...
const NEARBY = [stop('Nonneseter', 20), stop('Bergen stasjon', 400)];
...
async function run(label, hist, scheme) {
  ...
  await page.addInitScript(({ now, away, hist }) => {
    ...
    localStorage.setItem('default::t.homeLL', JSON.stringify(away));
  ...
  fs.mkdirSync('scratchpad/shots', { recursive: true });
  await page.screenshot({ path: 'scratchpad/shots/bergen-' + label + '-' + scheme + '.png', animations: 'disabled' });
  ...
  if (a.tilbake) {
    ...
    await page.screenshot({ path: 'scratchpad/shots/bergen-' + label + '-tilbake-' + scheme + '.png', animations: 'disabled' });
  }
  if (a.stripe) {
