 ...
 *
 * Rapportert fra tavla: «1 ANNEN MELDING · VIS» og «1 MELDING SKJULT · VIS»,
 * stablet.
 *
 * HVA BARE EN NETTLESER KAN AVGJØRE: at det faktisk BLIR én rad med begge
 * slagene i, at den bortlagte er til å lese når du folder ut, og at ↩ henter
 * tilbake bare den ene — ikke alt du noen gang har lagt bort.
 */
...
async function run(label, hist, scheme) {
  ...
  const les = () => page.evaluate(() => {
    const b = document.getElementById('auto-alerts');
    if (!b || b.style.display === 'none') return { rader: [], apne: [], bortlagt: 0 };
    return {
      rader: [...b.querySelectorAll('.alerts-more, .alerts-hidden, .alerts-other')]
        .map(e => e.className.replace(/ .*/, '') + ': ' + e.textContent.replace(/\s+/g, ' ').trim()),
      apne: [...b.querySelectorAll('.service-alert')].map(e =>
        (e.classList.contains('sa-put-away') ? '[bortlagt] ' : '')
        + e.textContent.replace(/\s+/g, ' ').trim().slice(0, 44)),
      alle: !!b.querySelector('.alerts-back-all'),
    };
  });

  console.log('\n══ ' + label + ' · ' + scheme + ' ══');
  let v = await les();
  v.rader.forEach(r => console.log('  ' + r));
  v.apne.forEach(a => console.log('      ' + a));

  const rad = page.locator('#auto-alerts .alerts-more').first();
  if (await rad.count()) {
    await rad.click();
    await page.waitForTimeout(500);
    v = await les();
    console.log('  — utfoldet —');
    v.rader.forEach(r => console.log('  ' + r));
    v.apne.forEach(a => console.log('      ' + a));
    console.log('      «hent alle tilbake»: ' + v.alle);
    await page.screenshot({ path: 'scratchpad/enrad-' + label + '-' + scheme + '.png', fullPage: true });

    // ↩ PÅ DEN FØRSTE BORTLAGTE: henter den tilbake, og bare den.
    const back = page.locator('#auto-alerts .sa-put-away .sa-back').first();
    if (await back.count()) {
      await back.click();
      await page.waitForTimeout(500);
      v = await les();
      console.log('  — etter ↩ på den første —');
      v.rader.forEach(r => console.log('  ' + r));
      v.apne.forEach(a => console.log('      ' + a));
      console.log('      lagt bort igjen: ' + await page.evaluate(() =>
        localStorage.getItem('default::t.alertHid')));
    }
  }
...
await run('meldinger', HIST, 'dark');
await run('meldinger', HIST, 'light');
