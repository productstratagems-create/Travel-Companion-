/**
 * Den samme meldingen, uansett hvilken vei den ble hentet.
 *
 * Rapportert: «Meldingshåndteringen er wonky.» Den var det på en målbar måte:
 * på en ren stopptavle mistet innsamlingen all herkomst, så `relevance` falt
 * gjennom til «vi vet ingenting» — som betyr MITT. Hver eneste melding sto
 * åpen som om den gjaldt deg.
 *
 * HVA BARE EN NETTLESER KAN AVGJØRE: at auto-reise og tavla nå sorterer den
 * samme lista likt, gjennom hver sin hentevei.
 */
...
async function run(label, hist, scheme) {
  ...
  const les = (id) => page.evaluate((bid) => {
    const b = document.getElementById(bid);
    if (!b || b.style.display === 'none') return { rad: '(tomt)', apne: [] };
    return {
      rad: (b.querySelector('.alerts-more') || {}).textContent
  ...
  console.log('\n══ ' + label + ' · ' + scheme + ' ══');
  const a = await les('auto-alerts');
  console.log('  AUTO-REISE  fold: ' + a.rad);
  a.apne.forEach(x => console.log('       åpen: ' + x));

  // SAMME LISTE, ANDRE VEI INN. Tavla henter den gjennom fetchBoard.
  // MÅL DET DU TROR DU MÅLER: hvilken skjerm står vi faktisk på, og finnes
  // banneret i det hele tatt? «(tomt)» kan like gjerne bety at prøven aldri
  // kom fram.
  const nav = await page.evaluate(() => [...document.querySelectorAll('.nav-item, .nav-btn, button')]
    .map(b => (b.textContent || '').replace(/\s+/g, ' ').trim()).filter(Boolean).slice(0, 12));
  console.log('  knapper: ' + JSON.stringify(nav));
  ...
  const hvor = await page.evaluate(() => ({
    ...
    finnes: !!document.getElementById('service-alerts'),
    vis: document.getElementById('service-alerts')
      ? document.getElementById('service-alerts').style.display : '-',
  }));
  console.log('  skjerm: ' + JSON.stringify(hvor));
  const t = await les('service-alerts');
  console.log('  TAVLA       fold: ' + t.rad);
  t.apne.forEach(x => console.log('       åpen: ' + x));
  await page.screenshot({ path: 'scratchpad/herkomst-' + label + '-' + scheme + '.png', fullPage: true });
