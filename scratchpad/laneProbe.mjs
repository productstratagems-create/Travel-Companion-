/**
 * Do the mode bands read better than one flat list?
 *
...
const DIST = path.join(ROOT, 'dist'); const PORT = 4499;
...
const stop = (name, m, lat0, cats) => ({
  properties: { id: 'NSR:StopPlace:' + name.replace(/\W/g, ''), name, label: name,
    category: cats || [name.endsWith(' T') ? 'metroStation' : 'onstreetBus'] },
...
/* The reported screen, at Hellerud: one metro stop, six kerbside bus stops,
   and Tveita T which is BOTH — the case the old `.find` could not express. */
const HELLERUD = [
  stop('Godlia', 114, OLD.lat, ['metroStation']),
  stop('Tveten gård', 376, OLD.lat, ['onstreetBus']),
  stop('Stordamveien', 721, OLD.lat, ['onstreetBus']),
  stop('Skøyenlia', 748, OLD.lat, ['onstreetBus']),
  stop('Tveita T', 751, OLD.lat, ['metroStation', 'onstreetBus']),
  stop('Trasoppveien', 774, OLD.lat, ['onstreetBus']),
  stop('Tveita', 776, OLD.lat, ['onstreetBus']),
  stop('Tveten bru', 799, OLD.lat, ['railStation']),
];
const FROM_OLD = [stop('Hellerud', 20, OLD.lat, ['metroStation', 'onstreetBus']), ...HELLERUD];
const FROM_NEW = FROM_OLD;
...
const lanes = () => page.$$eval('#v-auto .auto-lane', els => els.map(e => ({
  bane: (e.querySelector('.set-label') || {}).textContent || '?',
  stopp: Array.from(e.querySelectorAll('.auto-alt')).map(b =>
    b.querySelector('.nearby-name').textContent.trim() + ' · '
    + b.querySelector('.nearby-dist').textContent.trim()),
})));

// The list starts collapsed since v1.83.1 — open it the way a reader does.
await page.click('#auto-stop-toggle');
await page.waitForTimeout(600);

console.log('\n  ── b\u00e5ndene ──');
for (const l of await lanes()) {
  console.log('  ' + l.bane);
  l.stopp.forEach(x => console.log('      ' + x));
}
const rows = (await alts()).length;
const heading = await head();
console.log('\n  rader i alt   :', rows);
console.log('  overskriften  :', heading, '(skal telle STOPP, ikke rader)');

// A row in two lanes must still be one tap that picks one stop.
await page.click('#v-auto .auto-lane:nth-of-type(2) .auto-alt');
await page.waitForTimeout(800);
console.log('  etter trykk p\u00e5 en bussrad:', await head());

fs.mkdirSync('scratchpad/shots', { recursive: true });
await page.screenshot({ path: 'scratchpad/shots/lanes-dark.png', animations: 'disabled' });
await ctx.close();

const ctx2 = await browser.newContext({
  viewport: { width: 414, height: 860 }, deviceScaleFactor: 2, colorScheme: 'light',
  hasTouch: true, isMobile: true, timezoneId: 'Europe/Oslo', locale: 'nb-NO',
  geolocation: { latitude: NEW.lat, longitude: NEW.lon }, permissions: ['geolocation'],
});
const p2 = await ctx2.newPage();
await p2.addInitScript(({ now, old }) => {
  const Real = Date;
  class Pinned extends Real {
    constructor(...a) { super(...(a.length ? a : [now])); }
    static now() { return now; }
...
await p2.route('**/geocoder/**', r => r.fulfill({ status: 200, contentType: 'application/json',
  body: JSON.stringify({ features: FROM_NEW }) }));
await p2.route('**/journey-planner/**', route => route.fulfill({ status: 200,
  contentType: 'application/json',
  body: JSON.stringify({ data: { stopPlace: { id: 'x', name: 'x', estimatedCalls: CALLS },
    trip: { tripPatterns: [] } } }) }));
await p2.route(/tiles|open-meteo|overpass|valhalla|geoapify|mobility/, r => r.abort());
await p2.goto(`http://localhost:${PORT}/`, { waitUntil: 'load' });
await p2.waitForSelector('#auto-stop-toggle');
await p2.waitForTimeout(2000);
await p2.click('#auto-stop-toggle');
await p2.waitForTimeout(600);
await p2.screenshot({ path: 'scratchpad/shots/lanes-light.png', animations: 'disabled' });
await ctx2.close();
