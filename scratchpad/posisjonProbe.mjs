/**
 * Holdeplassen velges fra posisjonen appen godtok.
 *
 * Rapportert med skjermbilde: «DU ER VED Jernbanetorget · 10221 m å gå ·
 * 125 min gange · posisjonen er unøyaktig (±148 m)».
 *
 * HVA BARE EN NETTLESER KAN AVGJØRE: at en unøyaktig måling ti kilometer unna
 * faktisk ikke flytter overskriften på den ferdige skjermen — ikke bare at
 * geokoderen ikke blir spurt. Playwright kan flytte posisjonen underveis, så
 * den rapporterte hendelsen kan spilles av: stå på Hauketo, få én vill måling,
 * og se hva navnet sier.
 */
...
async function run(label, hist, scheme) {
  ...
  const head = () => page.evaluate(() => {
    ...
    return w ? w.textContent.replace(/\s+/g, ' ').trim().slice(0, 90) : '(ingen)';
  });

  console.log('\n══ ' + label + ' · ' + scheme + ' ══');
  console.log('  før : ' + await head());

  // DEN RAPPORTERTE HENDELSEN. Ti kilometer nord er omtrent Jernbanetorget,
  // med ±148 m — godt forbi ACC_GATE, altså en måling appen selv forkaster.
  await ctx.setGeolocation({ latitude: HERE.lat + 10000 / 111320, longitude: HERE.lon, accuracy: 148 });
  await page.waitForTimeout(2500);
  console.log('  etter vill måling (10 km, ±148 m): ' + await head());
  await page.screenshot({ path: 'scratchpad/vill-' + label + '-' + scheme + '.png', fullPage: true });

  // MOTPRØVEN: en GOD måling som faktisk flytter deg må fortsatt virke,
  // ellers har jeg bare gjort appen døv.
  await ctx.setGeolocation({ latitude: HERE.lat + 700 / 111320, longitude: HERE.lon, accuracy: 8 });
  await page.waitForTimeout(3000);
  console.log('  etter ekte flytting (700 m, ±8 m): ' + await head());
