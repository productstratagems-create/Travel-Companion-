 ...
 *
 * Den rapporterte skjermen fra Mortensrud, spilt av: 14 minutter å gå, en rad
 * med avganger om 3 · 18 · 33 min, og en nabo med bare én om 9 min.
 *
 * HVA BARE EN NETTLESER KAN AVGJØRE: at klokkeslettet under raden faktisk
 * hører til den avgangen som ikke er strøket over, og at raden du ikke rekker
 * noe på står uten klokke framfor å love en ankomst.
 */
...
const DIRS = [
  // Den rapporterte raden: 3 · 18 · 33 min. Bare de to siste er innenfor.
  { line: '3', front: 'Kolsås', mins: 3, stops: [pt(59.9139, 10.7522, 'Jernbanetorget')] },
  { line: '3', front: 'Kolsås', mins: 18, stops: [pt(59.9139, 10.7522, 'Jernbanetorget')] },
  { line: '3', front: 'Kolsås', mins: 33, stops: [pt(59.9139, 10.7522, 'Jernbanetorget')] },
  // Naboraden: én avgang om 9 min, altså ingenting å rekke.
  { line: '3', front: 'Stortinget', mins: 9, stops: [pt(59.9139, 10.7522, 'Jernbanetorget')] },
];
...
async function run(label, hist, scheme) {
  ...
  console.log('\n══ ' + label + ' · ' + scheme + ' ══');
  const rader = await page.evaluate(() => [...document.querySelectorAll('#auto-body .auto-dir')]
    .map(r => {
      let n = r.nextElementSibling; const stopp = [];
      while (n && n.classList.contains('auto-inline-stop')) {
        stopp.push(n.textContent.replace(/\s+/g, ' ').trim()); n = n.nextElementSibling;
      }
      const t = [...r.querySelectorAll('.auto-t-next, .auto-t-dim')]
        .map(e => (e.className.includes('missed') ? '[' : '') + e.textContent.trim()
          + (e.className.includes('missed') ? ']' : ''));
      return { rad: r.querySelector('.nearby-name').textContent.trim(),
        tider: t.join(' · '), stopp };
    }));
  console.log('  gange: ' + await page.evaluate(() =>
    (document.querySelector('#auto-where .auto-stop-facts') || {}).textContent));
  rader.forEach(r => {
    console.log('  ' + r.rad.padEnd(16) + r.tider);
    r.stopp.forEach(s => console.log('        ↳ ' + s));
  });
  await page.screenshot({ path: 'scratchpad/rekkevidde-' + label + '-' + scheme + '.png', fullPage: true });
...
await run('rekkevidde', HIST, 'dark');
await run('rekkevidde', HIST, 'light');
