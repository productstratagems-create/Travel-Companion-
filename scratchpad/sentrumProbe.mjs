...
const DIST = path.join(ROOT, 'dist'); const PORT = 4575;
...
async function run(label, hist, scheme) {
  ...
  const read = () => page.evaluate(() => {
    ...
    return {
      ...
      liste: Array.from(document.querySelectorAll('#auto-body .auto-side, #auto-body .auto-dir'))
  ...
  await page.screenshot({ path: 'scratchpad/sentrum-' + label + '-' + scheme + '.png' });
  await ctx.close();
}

await run('oslo', [], 'dark');
await run('oslo', [], 'light');
...
HERE.lat = 60.3913; HERE.lon = 5.3221;   // Bergen
await run('bergen', [], 'dark');
