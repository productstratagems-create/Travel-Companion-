/**
 * Serien av posisjoner, i en ekte nettleser.
 *
 * HVA BARE EN NETTLESER KAN AVGJØRE: at en gange faktisk bygger en serie
 * gjennom watchPosition (ikke bare gjennom en mock), at overskriften skifter
 * til holdeplassen du går MOT, at den ikke blafrer mens du går mellom to, og
 * at setningen «du går mot den» er til å lese i begge temaer.
 *
 * Playwright flytter posisjonen med setGeolocation, og appen får den gjennom
 * sin egen watchPosition — altså samme vei som på en telefon.
 */
...
// To holdeplasser, like langt unna, hver sin vei: går du nordover nærmer du
// deg den ene og går fra den andre. Uten serien skiller ingenting dem.
// Sørstopp er NÆRMEST i ro, så appen navngir den — og går du nordover skal
// Nordstopp ta over. Uten dette valgte fiksturen Nordstopp fra start, og
// prøven viste at setningen kom, ikke at byttet skjedde.
const NEARBY = [stop('Nordstopp', 320), stop('Sørstopp', -180)];
...
async function run(label, hist, scheme) {
  ...
  const head = () => page.evaluate(() => {
    const w = document.getElementById('auto-where');
    return w ? w.textContent.replace(/\s+/g, ' ').trim().slice(0, 95) : '(ingen)';
  });
  const navn = () => page.evaluate(() => {
    const h = document.querySelector('#auto-where .auto-stop-name, #auto-where h2, #auto-where .set-value');
    return h ? h.textContent.trim() : (document.getElementById('auto-where') || {}).textContent;
  });

  console.log('\n══ ' + label + ' · ' + scheme + ' ══');
  console.log('  i ro   : ' + await head());

  // GÅ NORDOVER, i gangfart, gjennom appens egen watchPosition.
  const M = 1 / 111320;
  const sett = [];
  for (let i = 1; i <= 12; i++) {
    await ctx.setGeolocation({ latitude: HERE.lat + i * 4 * M, longitude: HERE.lon, accuracy: 8 });
    await page.waitForTimeout(900);
    sett.push((await navn()).replace(/\s+/g, ' ').slice(0, 24));
  }
  console.log('  går nord: ' + await head());
  // BLAFRER DET? Hvor mange ganger skiftet navnet underveis.
  let skift = 0;
  for (let i = 1; i < sett.length; i++) if (sett[i] !== sett[i - 1]) skift++;
  console.log('  navneskift underveis: ' + skift + '  ' + JSON.stringify([...new Set(sett)]));
  await page.screenshot({ path: 'scratchpad/spor-' + label + '-' + scheme + '.png', fullPage: true });
...
await run('posisjon', HIST, 'dark');
await run('posisjon', HIST, 'light');
