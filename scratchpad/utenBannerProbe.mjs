/**
 * Tavla viser ingen meldinger — og de andre skjermene gjør det fortsatt.
 *
 * «Kutt all meldingsvisning øverst på tavle. Den oppfører seg uansett feil.»
 *
 * HVA BARE EN NETTLESER KAN AVGJØRE: at #service-alerts ikke finnes i DOM-en
 * i det hele tatt, OG at auto-reise fortsatt har sitt banner i samme kjøring.
 * «Meldingene finnes fortsatt» skal være målt, ikke antatt — det er den dyre
 * feilen her å rive ut innsamlingen sammen med visningen.
 *
 * Merk: denne prøven når ikke tavlas banner med en ekte melding (se
 * scratchpad/tavlaProbe.mjs for hvorfor). Her er det motsatte som måles — at
 * det ikke finnes noe å nå — og det kan den se.
 */
...
async function run(label, hist, scheme) {
  ...
  const auto = await page.evaluate(() => {
    ...
    return {
      finnes: !!b,
      vises: b ? b.style.display !== 'none' : false,
      meldinger: b ? b.querySelectorAll('.service-alert').length : 0,
      fold: b && b.querySelector('.alerts-more')
        ? b.querySelector('.alerts-more').textContent.replace(/\s+/g, ' ').trim() : '-',
    };
  });
  console.log('  AUTO-REISE: banner ' + (auto.vises ? 'vises' : 'skjult')
    + ' · ' + auto.meldinger + ' melding(er) · fold: ' + auto.fold);

  // TAVLA: ingenting skal finnes å vise.
  await page.evaluate(() => { const b = [...document.querySelectorAll('*')]
    .find(e => /TAVLA/.test(e.textContent || '') && e.children.length === 0); if (b) b.click(); });
  await page.waitForTimeout(2500);
  const tavla = await page.evaluate(() => ({
    paaTavla: (() => { const e = document.getElementById('v-board');
      return !!e && e.style.display !== 'none'; })(),
    banner: document.getElementById('service-alerts') ? 'FINNES' : 'finnes ikke',
    meldingsbokser: document.querySelectorAll('#v-board .service-alert').length,
    folder: document.querySelectorAll('#v-board .alerts-more').length,
  }));
  console.log('  TAVLA: på skjermen=' + tavla.paaTavla + ' · #service-alerts ' + tavla.banner
    + ' · bokser ' + tavla.meldingsbokser + ' · folder ' + tavla.folder);
  await page.screenshot({ path: 'scratchpad/uten-' + label + '-' + scheme + '.png', fullPage: true });
...
await run('banner', HIST, 'dark');
await run('banner', HIST, 'light');
